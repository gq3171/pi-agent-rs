        ...
        match msg {
                ...
                for block in &a.content {
                    match block {
                        ...
                        ContentBlock::ToolCall(tc) => {
                            let sig =
                                resolve_thought_signature(same, tc.thought_signature.as_deref());
            ...
            Message::ToolResult(tr) => {
                ...
                if has_images && supports_multimodal {
                    fn_resp["parts"] = json!(image_parts);
                }
                if requires_tool_call_id(&model.id) {
                    fn_resp["id"] = json!(tr.tool_call_id);
                }
...
pub fn extract_retry_delay(
    ...
    if let Some(hdrs) = headers {
        if let Some(retry_after) = hdrs.get("retry-after").and_then(|v| v.to_str().ok()) {
            if let Ok(secs) = retry_after.parse::<f64>() {
                if secs.is_finite() {
                    if let Some(d) = normalize_delay(secs * 1000.0) {
                        return Some(d);
                    }
                }
            }
            if let Ok(date) = chrono::DateTime::parse_from_rfc2822(retry_after) {
                let diff =
                    (date.signed_duration_since(chrono::Utc::now())).num_milliseconds() as f64;
                if let Some(d) = normalize_delay(diff) {
                    return Some(d);
                }
            }
        }
    ...
    fn re1() -> &'static Regex {
        ...
        static RE: OnceLock<Regex> = OnceLock::new();
        RE.get_or_init(|| {
            Regex::new(r"(?i)reset after (?:(\d+)h)?(?:(\d+)m)?(\d+(?:\.\d+)?)s").unwrap()
        })
    }
    if let Some(c) = re1().captures(error_text) {
        ...
        if let Some(sm) = c.get(3) {
            if let Ok(s) = sm.as_str().parse::<f64>() {
                if !s.is_nan() {
                    if let Some(d) = normalize_delay(((h * 60.0 + m) * 60.0 + s) * 1000.0) {
                        return Some(d);
                    }
                }
            }
        ...
        if let (Some(v), Some(u)) = (c.get(1), c.get(2)) {
            if let Ok(val) = v.as_str().parse::<f64>() {
                if !val.is_nan() && val > 0.0 {
                    let ms = if u.as_str().eq_ignore_ascii_case("ms") {
                        val
                    } else {
                        val * 1000.0
                    };
                    if let Some(d) = normalize_delay(ms) {
                        return Some(d);
                    }
                }
            }
...
pub fn build_request(
    ...
    if let Some(mt) = options.base.max_tokens {
...
async fn stream_response_owned(
    ...
    loop {
        ...
        for line in &lines {
            ...
            if let Some(candidate) = response_data
                .get("candidates")
                    ...
                    .and_then(|p| p.as_array())
                {
                    for part in parts {
                        ...
                        if let Some(fc) = part.get("functionCall") {
                            ...
                            let fc_id =
                                fc.get("id").and_then(|i| i.as_str()).map(|s| s.to_string());
                            let sig = part
                            ...
                            let needs_new_id = match &fc_id {
                                None => true,
                                Some(id) => output.content.iter().any(
                                    |b| matches!(b, ContentBlock::ToolCall(tc) if tc.id == *id),
                                ),
                            };
...
async fn run_stream(
    ...
    let api_key_raw = options.base.api_key.as_deref().ok_or(
    ...
    let creds: Creds = serde_json::from_str(api_key_raw).map_err(|_| {
    ...
    let endpoints: Vec<String> = if !base_url.is_empty()
    ...
    let provider_hdrs = if is_antigravity {
        antigravity_headers()
    } else {
        gemini_cli_headers()
    };
    ...
    for attempt in 0..=MAX_RETRIES {
        ...
        match req.send().await {
                        ...
                        if let Some(s) = sd {
                            if s > max_d {
                                return Err(format!(
                                    ...
                                    (s as f64 / 1000.0).ceil() as u64,
                                    (max_d as f64 / 1000.0).ceil() as u64,
                                    extract_error_message(&et)
                                ));
                            }
                        }
                    }
            ...
            Err(e) => {
                last_err = Some(if let Some(src) = e.source() {
                    format!("Network error: {src}")
                } else {
                    format!("Network error: {e}")
                });
                if attempt < MAX_RETRIES {
                    cancellable_sleep(BASE_DELAY_MS * 2u64.pow(attempt), cancel).await?;
                    continue;
                }
    ...
    let initial_resp =
        ok_response.ok_or_else(|| last_err.unwrap_or("Failed after retries".to_string()))?;

    // --- Empty stream retry loop ---
    // We make requests and stream them one at a time. The first attempt uses initial_resp.
    let make_request =
        |client: &reqwest::Client, url: &str, headers: &HashMap<String, String>, body: &str| {
...
pub fn stream_simple_google_gemini_cli(
    ...
    if model.id.contains("3-pro") || model.id.contains("3-flash") {
        return stream_google_gemini_cli(
            ...
            &GoogleGeminiCliOptions {
                ...
                thinking_budget_tokens: None,
                thinking_level: Some(get_gemini_cli_thinking_level(&effort, &model.id)),
                project_id: None,
            },
            cancel,
        );
    ...
    stream_google_gemini_cli(
        ...
        &GoogleGeminiCliOptions {
            ...
            tool_choice: None,
            thinking_enabled: true,
            thinking_budget_tokens: Some(budget as i64),
            thinking_level: None,
            project_id: None,
        },
        cancel,
...
mod tests {
    ...
    fn test_extract_retry_delay_from_text() {
        ...
        assert_eq!(
            extract_retry_delay("Please retry in 500ms", None),
            Some(1500)
        );
        assert_eq!(
            extract_retry_delay(r#""retryDelay": "10.5s""#, None),
            Some(11500)
        );
    ...
    fn test_extract_error_message() {
        ...
        assert_eq!(
            extract_error_message(r#"{"status":"error"}"#),
            r#"{"status":"error"}"#
        );
    }
    ...
    fn test_build_request_basic() {
        ...
        assert_eq!(
            req["request"]["systemInstruction"]["parts"][0]["text"],
            "You are helpful."
        );
    ...
    fn test_build_request_tools_claude_uses_parameters() {
        ...
        let ctx = Context {
            system_prompt: None,
            messages: vec![Message::User(UserMessage {
                content: UserContent::Text("search".to_string()),
                timestamp: 0,
            })],
            tools: Some(vec![Tool {
                name: "s".to_string(),
        ...
        assert_eq!(
            req["request"]["toolConfig"]["functionCallingConfig"]["mode"],
            "AUTO"
        );
    }
    ...
    fn test_get_gemini_cli_thinking_level() {
        assert_eq!(
            get_gemini_cli_thinking_level(&ThinkingLevel::Minimal, "gemini-3-pro"),
            "LOW"
        );
        assert_eq!(
            get_gemini_cli_thinking_level(&ThinkingLevel::Low, "gemini-3-pro"),
            "LOW"
        );
        assert_eq!(
            get_gemini_cli_thinking_level(&ThinkingLevel::Medium, "gemini-3-pro"),
            "HIGH"
        );
        assert_eq!(
            get_gemini_cli_thinking_level(&ThinkingLevel::High, "gemini-3-pro"),
            "HIGH"
        );
        assert_eq!(
            get_gemini_cli_thinking_level(&ThinkingLevel::Minimal, "gemini-3-flash"),
            "MINIMAL"
        );
        assert_eq!(
            get_gemini_cli_thinking_level(&ThinkingLevel::Low, "gemini-3-flash"),
            "LOW"
        );
        assert_eq!(
            get_gemini_cli_thinking_level(&ThinkingLevel::Medium, "gemini-3-flash"),
            "MEDIUM"
        );
        assert_eq!(
            get_gemini_cli_thinking_level(&ThinkingLevel::High, "gemini-3-flash"),
            "HIGH"
        );
        assert_eq!(
            get_gemini_cli_thinking_level(&ThinkingLevel::Low, "gemini-2.5-flash"),
            "LOW"
        );
    }
